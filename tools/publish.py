...
# Uploads cfpu relases to workshop
...
def buildCompatFolder(folderName, copyFileNames):
    ...
    for copyFileName in copyFileNames:
        for file in os.listdir(cfpu_optionals_dir):
            if fnmatch.fnmatch(file, copyFileName):
                print("    Copying: {}".format(file))
                shutil.copyfile(os.path.join(cfpu_optionals_dir, file), os.path.join(compatRelease_dir, "addons", file))
...
#GLOBALS
release_dir = "P:\\x\\cfpu\\release"
project = "@cfpu"
publisherTool_path = find_bi_tools()
changelog_path = os.path.join(release_dir,"changelog.txt")
cfpu_release_dir = os.path.join(release_dir, project)
cfpu_optionals_dir = os.path.join(cfpu_release_dir, "optionals")
...
def main(argv):
    if not os.path.exists(cfpu_release_dir):
        raise Exception("cfpu_release_dir not found","cfpu not built or in wrong path")
    if not os.path.exists(cfpu_optionals_dir):
        raise Exception("cfpu_optionals_dir not found","cfpu not built or in wrong path")
